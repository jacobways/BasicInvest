...
module.exports = async (req, res) => {
  ...
  const lastObj = await ForeignTradeTrend_Kospi.max('date')
  const lastDate = getDate(String(lastObj))

  const endObj = await ForeignTradeTrend_Kospi.findAll({
    where: {
      date: {
  ...
  const nextObj = await ForeignTradeTrend_Kospi.max('date', {where: { date: {[Op.lt]: getYearMonth(lastDate)}}})
  const nextDate = getDate(String(nextObj))

  const middleObj = await ForeignTradeTrend_Kospi.findAll({
    where: {
      date: {
  ...
  const earlyObj = await ForeignTradeTrend_Kospi.max('date', {where: { date: {[Op.lt]: getYearMonth(nextDate)}}})
  const earlyDate = getDate(String(earlyObj))

  const startObj = await ForeignTradeTrend_Kospi.findAll({
    where: {
      date: {
